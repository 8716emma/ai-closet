...
const CLOSER_DATA = {
  ...
  templates: {
    "캐주얼": {
      ...
      items: ["오버핏 드롭숄더 스웨트셔츠", "생지 와이드 핏 코튼 데님 팬츠", "레트로 빈티지 코트 스니커즈", "브라운 나일론 메신저 백"],
    ...
    "꾸안꾸": {
      ...
      items: ["네추럴 오버핏 골지 니트 가디건", "크림 아이보리 핀턱 슬랙스", "베이지 톤의 스웨이드 뮬/러너 스니커즈", "심플한 가죽 카드 홀더"],
    ...
    "오피스": {
      ...
      items: ["브라운 싱글 테일러드 블레이저 재킷", "프리미엄 노아이론 클래식 셔츠", "차콜 그레이 테이퍼드 핏 슬랙스", "더블 몽크 스트랩 로퍼 / 가죽 브리프케이스"],
    ...
    "트레이닝": {
      ...
      items: ["세미 루즈핏 기능성 나일론 아노락 재킷", "테크니컬 스트레치 조거 트랙 팬츠", "쿠셔닝이 풍부한 러닝 트레이너 슈즈", "화이트 스포츠 볼캡"],
    ...
    "남친·여친룩": {
      ...
      items: ["케이블 꽈배기 라운드 니트웨어", "단정한 하이웨이스트 크림 코튼 팬츠", "깔끔한 브라운 가죽 첼시 부츠", "미니멀한 스퀘어 가죽 숄더백"],
    ...
    "단정해": {
      ...
      items: ["스탠다드 울 모크넥 니트 풀오버", "스트레이트 슬림핏 미드블루 데님", "다크 브라운 풀 그레인 가죽 로퍼", "미니멀 스퀘어 레더 벨트"],
    ...
    "수트": {
      ...
      items: ["테일러드 노치드 라펠 수트 재킷", "울 블렌드 플랫 프런트 정장 팬츠", "은은한 광택의 옥스포드 가죽 드레스 슈즈", "클래식 실크 타이"],
    ...
    "Y2K": {
      ...
      items: ["레트로 크롭 빈티지 레터링 그래픽 티", "로우라이즈 와이드 멀티카고 팬츠", "볼드 플랫폼 청키 청크 스니커즈", "Y2K 메탈릭 미니 숄더백"],
    ...
    "빈티지": {
      ...
      items: ["자카드 아가일 패턴 오버핏 가디건", "빈티지 워싱 테이퍼드 코듀로이 바지", "클래식 스웨이드 워크 부츠", "레트로 브라운 안경테"],
    ...
    "default": {
      ...
      items: ["소프트 아이보리 하프넥 파인울 니트", "코코아 브라운 플랫 핀턱 치노 팬츠", "스웨이드 소재 로퍼 단화", "심플한 가죽 스트랩 워치"],
      links: "musinsa.com"
    }
  },
...
반드시 아래의 JSON 규격(형식)만을 출력해야 해. 마크다운(\`\`\`json) 등 부가 텍스트는 절대 붙이지 말고 순수 JSON 문자열만 출력해줘:
{
  ...
  "items": ["의상 아이템 1", "의상 아이템 2", "의상 아이템 3", "의상 아이템 4"],
