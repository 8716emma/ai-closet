...
DATA_EXCEPTIONS = ['slide-styles-data.js', 'closet-recommender-data.js']  # 순수 데이터 파일 예외
...
if os.path.exists(dc_path):
    ...
    for preset in EXPECTED_PRESETS:
        # PRESETS 객체에 키가 존재하는지
        has_preset_def = f"'{preset}'" in dc_content or f'"{preset}"' in dc_content or f'{preset}:' in dc_content
        # 버튼에서 호출하는지
        has_preset_btn = f"applyPreset('{preset}')" in dc_content
        check(f"프리셋 '{preset}' 정의", has_preset_def, "PRESETS 객체에 키 없음")
        check(f"프리셋 '{preset}' 버튼", has_preset_btn, "버튼 onclick 없음")
    
    # 각 프리셋에 prompt 필드 있는지
    preset_prompts = re.findall(r"prompt:\s*\"", dc_content)
...
# design-controller의 프리셋에 Pretendard 폴백 있는지
if os.path.exists(dc_path):
    dc_content = read_file(dc_path)
    # 모든 프리셋 font 필드에 Pretendard가 포함되는지
    font_fields = re.findall(r"font:\s*\"([^\"]+)\"", dc_content)
...
# GEMINI.md에서 docs/index.md 참조하는지
gemini_path = os.path.join(BASE, '.agents', 'GEMINI.md')
if os.path.exists(gemini_path):
    gemini_content = read_file(gemini_path)
    check("GEMINI.md → docs/index.md 참조", 'docs/index.md' in gemini_content,
          "GEMINI에서 SSoT 참조 없음")
    check("GEMINI.md → docs/log.md 참조", 'docs/log.md' in gemini_content,
          "GEMINI에서 로그 참조 없음")
